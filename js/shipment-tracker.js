    ...
    async function renderStageView() {
        ...
        if (currentStageData.stageName === 'bills') {
            if (buttonContainer) {
                ...
                const freightForwarderButton = document.createElement('button');
                freightForwarderButton.textContent = 'Manage Freight Forwarder';
                freightForwarderButton.onclick = openFreightForwarderModal;
                freightForwarderButton.classList.add('button-secondary', 'ml-2');
                buttonContainer.appendChild(freightForwarderButton);

                const fbrDutyButton = document.createElement('button');
                fbrDutyButton.textContent = 'Manage FBR Duty';
                fbrDutyButton.onclick = openFbrDutyModal;
                fbrDutyButton.classList.add('button-secondary', 'ml-2');
                buttonContainer.appendChild(fbrDutyButton);

                const bilityButton = document.createElement('button');
                bilityButton.textContent = 'Manage Bility';
                bilityButton.onclick = openBilityModal;
                bilityButton.classList.add('button-secondary', 'ml-2');
                buttonContainer.appendChild(bilityButton);

                const clearingAgentBillButton = document.createElement('button');
                clearingAgentBillButton.textContent = 'Manage Clearing Agent Bill';
                clearingAgentBillButton.onclick = openClearingAgentBillModal;
                clearingAgentBillButton.classList.add('button-secondary', 'ml-2');
                buttonContainer.appendChild(clearingAgentBillButton);

                const costingSheetButton = document.createElement('button');
                costingSheetButton.textContent = 'View Professional Costing Sheet';
                costingSheetButton.onclick = () => window.open(`costing-sheet-enhanced.html?id=${shipmentId}`, '_blank');
                costingSheetButton.classList.add('button-secondary', 'ml-2');
                costingSheetButton.style.background = 'var(--success-color)';
                costingSheetButton.style.color = 'white';
                buttonContainer.appendChild(costingSheetButton);
            }
        }
    ...
    async function renderStageEdit() {
        ...
        if (currentStageData.stageName === 'freight_query') {
            console.log('🔄 Checking if auto-mapping is needed for freight_query...');
            
            // Check if the key mapped fields are empty
            const readinessField = document.getElementById('readiness_date');
            const grossWeightField = document.getElementById('gross_weight');
            const cartoonsField = document.getElementById('no_of_cartoons');
            
            const needsMapping = (!readinessField?.value && !grossWeightField?.value && !cartoonsField?.value) ||
                                (!data || !data.readiness_date);
            
            if (needsMapping) {
                console.log('🔄 Auto-mapping data from supplier_shipment_details to freight_query...');
                try {
                    ...
                    if (supplierError) {
                        console.warn('⚠️ Could not load supplier shipment details for auto-mapping:', supplierError.message);
                    } else if (supplierData) {
                        console.log('✅ Found supplier data for auto-mapping:', supplierData);
                        ...
                        Object.keys(mappings).forEach(fieldName => {
                            ...
                            if (fieldElement && mappings[fieldName] !== null && mappings[fieldName] !== undefined) {
                                fieldElement.value = mappings[fieldName];
                                console.log(`📋 Auto-mapped ${fieldName}: ${mappings[fieldName]}`);
                            }
                        });

                        // Set default sent_at to current datetime
                        const sentAtField = document.getElementById('sent_at');
                        if (sentAtField && !sentAtField.value) {
                            const now = new Date();
                            sentAtField.value = now.toISOString().slice(0, 16); // Format for datetime-local
                            console.log('📅 Set default sent_at to current time');
                        }
                    }
                } catch (error) {
                ...
                console.log('ℹ️ Auto-mapping skipped - fields already have values');
            }
        }
    ...
    async function saveFbrDuty() {
        ...
        const fbrDutyFormData = {
            ...
            l_single_declaration_rate: parseFloat(document.getElementById('fbr_l_single_declaration_rate').value) || null,
            l_single_declaration_amount: parseFloat(document.getElementById('fbr_l_single_declaration_amount').value) || null,
            m_release_order_rate: parseFloat(document.getElementById('fbr_m_release_order_rate').value) || null,
            m_release_order_amount: parseFloat(document.getElementById('fbr_m_release_order_amount').value) || null,
            n_stamp_duty_rate: parseFloat(document.getElementById('fbr_n_stamp_duty_rate').value) || null,
        ...
        let fbrDutyId = document.getElementById('fbr_duty_id').value;

        if (fbrDutyId) {
            const { error } = await supabase.from('fbr_duty').update(fbrDutyFormData).eq('id', fbrDutyId);
            if (error) {
                showMessage('fbr-duty-modal-message', `Error updating FBR duty: ${error.message}`, true);
                return;
            }
        } else {
            const { data, error } = await supabase.from('fbr_duty').insert(fbrDutyFormData).select().single();
            if (error) {
                showMessage('fbr-duty-modal-message', `Error creating FBR duty: ${error.message}`, true);
                return;
            }
            fbrDutyId = data.id;
            document.getElementById('fbr_duty_id').value = fbrDutyId;
        }
    ...
    function populateSupplierDetails(supplier, stats, shipmentData) {
        ...
        if (supplier.is_lc_shared) {
            ...
            // Only show LC sharing section if shipment is in LC stages
            const currentStage = shipmentData.current_stage;
    ...
    function viewSupplierShipments() {
        if (window.currentSupplierId) {
            window.open(`shipments.html?supplier=${window.currentSupplierId}`, '_blank');
        } else {
            showToast('Supplier information not available', false);
        }
    }
